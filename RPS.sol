...
contract RockPaperScissors {
    ...
    enum Resultats {None, PlayerA, PlayerB, Draw}   // Possible Resultats
    ...
    function getResultat() public revealPhaseEnded returns (Resultats) {
        ...
        pay(addrA, addrB, betPlayerA, resultat);

        return resultat;
    }

    // Pay the winner(s).
    function pay(address payable addrA, address payable addrB, uint betPlayerA, Resultats resultat) private {
        // Uncomment lines below if you need to adjust the gas limit
        if (resultat == Resultats.PlayerA) {
            addrA.transfer(address(this).balance);
        } else if (resultat == Resultats.PlayerB) {
